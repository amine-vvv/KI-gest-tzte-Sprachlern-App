...
function App() {
  ...
  const [wordExplanation, setWordExplanation] = useState(null); // Changed to null to indicate no explanation yet
  ...
  const [quizResults, setQuizResults] = useState(null); // { score: number, total: number, feedback: string }
  const [isGeneratingQuiz, setIsGeneratingQuiz] = useState(false);

  // New state for sentence rephrasing
  const [sentenceToRephrase, setSentenceToRephrase] = useState('');
  const [rephrasedSentenceData, setRephrasedSentenceData] = useState(null); // { original: string, rephrased: string, explanation: string }
  ...
  const [grammarAnalysis, setGrammarAnalysis] = useState(null); // { original: string, corrected: string, errors: [{error: string, explanation: string}] }
  ...
  const [culturalInsight, setCulturalInsight] = useState(null); // Corrected: Must use useState for state initialization
  ...
  const [keyVocabulary, setKeyVocabulary] = useState([]); // [{ word: string, translation: string, explanation: string }]
  ...
  const [pronunciationGuide, setPronunciationGuide] = useState(null); // { phonetic: string, tips: string, exampleSentence: string, exampleSentenceTranslation: string }
  ...
  const [flashcards, setFlashcards] = useState([]); // [{ word: string, translation: string, definition: string, exampleSentence: string }]
  ...
  // This ref will now store segments (words and spaces) with their original indices
  ...
  const [wordModalWidth, setWordModalWidth] = useState(512); // Default max-w-lg is 512px
  ...
  const [quizModalWidth, setQuizModalWidth] = useState(768); // Default max-w-2xl is 768px
  ...
  const [errorModalWidth, setErrorModalWidth] = useState(384); // Default max-w-sm is 384px
  ...
  const updateVoicesAndSetDefault = () => {
    ...
    if (voices.length > 0) {
      // Prioritize finding a voice that exactly matches the target language
      ...
      // If no exact match, try to find one that starts with the target language code
      if (!preferredVoice) {
      ...
      setSpeechVoice(null); // No voices available
    }
  };

  // Effect to load API key from localStorage and initialize voices on component mount
  useEffect(() => {
    ...
    updateVoicesAndSetDefault(); // Call once immediately in case voices are already loaded

    return () => {
      if (speechSynthesisRef.current) {
        speechSynthesisRef.current.cancel(); // Ensure any ongoing speech is stopped on unmount
        speechSynthesisRef.current.onvoiceschanged = null; // Clean up the event listener
      }
    };
  }, [targetLanguage]); // Re-run when target language changes to update default voice

  // Effect to scroll to the bottom of the dialogue chat when new messages arrive
  useEffect(() => {
  ...
  // Effect to scroll to the bottom of the role play chat when new messages arrive
  useEffect(() => {
  ...
  // Custom Hook for Modal Resizing and Dragging Logic
  const useModalInteractionHooks = (
    ...
    minHeight,
    defaultWidth,
    defaultHeight // Can be 'auto' or a number
  ) => {
    ...
    useEffect(() => {
      const initialPositioning = () => {
        if (modalRef.current) {
          // Get the actual rendered dimensions if defaultHeight is 'auto'
          ...
          const effectiveHeight = defaultHeight === 'auto' ? currentModalRect.height : currentModalRect.height;

          // Only center if it's currently at (0,0) or off-screen, implying it hasn't been dragged/positioned yet
          // Check if left/top are explicitly set to 0 or are empty strings (initial state)
      ...
      // Use a small timeout to ensure modal is rendered and has dimensions before centering
  ...
  // Initialize resizing and dragging hooks for all modals
  const { startResize: startWordModalResize, startDrag: startWordModalDrag } = useModalInteractionHooks(setWordModalWidth, setWordModalHeight, setWordModalX, setWordModalY, isWordModalResizing, setIsWordModalResizing, isWordModalDragging, setIsWordModalDragging, wordModalRef, 300, 200, 512, 400);
  const { startResize: startQuizModalResize, startDrag: startQuizModalDrag } = useModalInteractionHooks(setQuizModalWidth, setQuizModalHeight, setQuizModalX, setQuizModalY, isQuizModalResizing, setIsQuizModalResizing, isQuizModalDragging, setIsQuizModalDragging, quizModalRef, 400, 300, 768, 600);
  ...
  const { startResize: startErrorModalResize, startDrag: startErrorModalDrag } = useModalInteractionHooks(setErrorModalWidth, setErrorModalHeight, setErrorModalX, setErrorModalY, isErrorModalResizing, setIsErrorModalResizing, isErrorModalDragging, setIsErrorModalDragging, errorModalRef, 250, 150, 384, 250);
  ...
  const showError = (message) => {
    ...
    // Reset error modal size and position to default when opened
    setErrorModalWidth(384);
    setErrorModalHeight('auto');
    setErrorModalX((window.innerWidth - 384) / 2);
    setErrorModalY((window.innerHeight - 250) / 2); // Using a default height for centering
  };
  ...
  const handleWordClick = async (word) => {
    ...
    const currentTargetLanguageName = languageMap[targetLanguage] || targetLanguage;
    // Updated prompt to request explanation in target language AND Arabic explanation
    const prompt = `Provide a detailed explanation of the word '${word}' in ${currentTargetLanguageName}. Also, provide a separate detailed explanation of the same word '${word}' entirely in Arabic. Additionally, provide synonyms in ${currentTargetLanguageName}, antonyms in ${currentTargetLanguageName}, three example sentences in ${currentTargetLanguageName} using the word, and the direct translation of the word '${word}' into Arabic. Format the response as a JSON object with the following keys: 'explanation' (this is the detailed explanation in ${currentTargetLanguageName}), 'arabicExplanation' (this is the detailed explanation in Arabic), 'synonyms' (a string listing synonyms in ${currentTargetLanguageName}, comma-separated if multiple), 'antonyms' (a string listing antonyms in ${currentTargetLanguageName}, comma-separated if multiple), 'examples' (an array of strings, each being an example sentence in ${currentTargetLanguageName}), and 'arabicTranslation' (this is the direct Arabic translation of the word '${word}').`;
    ...
    try {
      ...
      const payload = {
        ...
        generationConfig: {
          ...
          responseSchema: { // Updated schema
            type: "OBJECT",
            properties: {
              explanation: { type: "STRING" }, // In target language
              arabicExplanation: { type: "STRING" }, // In Arabic
              synonyms: { type: "STRING" }, // In target language
              antonyms: { type: "STRING" }, // In target language
              examples: { type: "ARRAY", items: { type: "STRING" } }, // In target language
              arabicTranslation: { type: "STRING" } // Word translated to Arabic
            },
  ...
  // Function to rephrase a sentence using Gemini API
  ...
  // Function to analyze grammar using Gemini API
  ...
  // Function to get sentence completions using Gemini API
  ...
  // Function to get cultural insights using Gemini API
  ...
  // Function to summarize story using Gemini API
  ...
  // Function to extract key vocabulary using Gemini API
  ...
  // Function to start conversation practice
  const startDialoguePractice = async () => {
    ...
    setIsRespondingToDialogue(true); // Set loading state
    ...
    // Prompt for AI to generate an initial greeting in the target language
    ...
    try {
        ...
        if (result.candidates && result.candidates.length > 0 &&
            ...
            // Fallback if AI fails to generate initial prompt
        ...
        setIsRespondingToDialogue(false); // Clear loading state
    }
};

  // Function to send dialogue message and get AI response
  ...
  const getPronunciationGuide = async () => {
    ...
    setPronunciationModalY((window.innerHeight - 400) / 2);
  ...
  const startRolePlayScenario = async () => {
    ...
    // System prompt for the AI's persona and language to generate the initial message
  ...
  const generateFlashcards = async () => {
    ...
    setFlashcardModalY((window.innerHeight - 450) / 2);
    ...
    <div className={`min-h-screen font-sans ${isDarkMode ? 'bg-gray-900 text-gray-100' : 'bg-gray-100 text-gray-900'}`}>
      ...
      {selectedWordForExplanation ? (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50">
          <div
            ref={wordModalRef}
            className={`absolute p-8 rounded-xl shadow-2xl ${isDarkMode ? 'bg-gray-800 text-gray-100' : 'bg-white text-gray-900'}`}
            style={{
          ...
          >
            {/* Drag Handle */}
            <div onMouseDown={startWordModalDrag} className="absolute top-0 left-0 right-0 h-10 cursor-grab rounded-t-xl z-10"></div>
            <h2 className="text-2xl font-bold mb-4 text-indigo-600 relative z-20" dir="rtl">
              شرح الكلمة: <span dir="ltr" className="inline-block text-left">{selectedWordForExplanation}</span>
            </h2>
            {isExplainingWord ? (
              ...
              <div className="overflow-y-auto" style={{ maxHeight: wordModalHeight === 'auto' ? 'none' : `${wordModalHeight - 150}px` }}>
                ...
                <p dir={targetLanguage === 'ar' ? 'rtl' : 'ltr'} className={`mb-3 p-2 rounded ${isDarkMode ? 'bg-gray-700' : 'bg-gray-100'} ${targetLanguage === 'ar' ? 'text-right' : 'text-left'}`}>
                    {wordExplanation.explanation}
                </p>

                {/* Explanation in Arabic */}
                {wordExplanation.arabicExplanation && (
                    <>
                        <p className="mb-1 mt-3" dir="rtl"> 
                          <span className="font-semibold">الشرح (بالعربية):</span>
                        </p>
                        <p dir="rtl" className={`mb-3 p-2 rounded ${isDarkMode ? 'bg-gray-700' : 'bg-gray-100'} text-right`}>
                            {wordExplanation.arabicExplanation}
                        </p>
                    </>
                )}
                ...
                <p className="mb-1 mt-3" dir="rtl">
                 ...
                 <p dir="rtl" className={`mb-3 p-2 rounded ${isDarkMode ? 'bg-gray-700' : 'bg-gray-100'} text-right`}>
                    {wordExplanation.arabicTranslation}
                </p>
                <p className="font-semibold mt-4" dir="rtl">أمثلة ({languageMap[targetLanguage] || targetLanguage}):</p>
                <ul className={`list-disc list-outside ${targetLanguage === 'ar' ? 'mr-6' : 'ml-6'} mt-1`} dir={targetLanguage === 'ar' ? 'rtl' : 'ltr'}>
                  {wordExplanation.examples && wordExplanation.examples.map((example, idx) => (
                    <li key={idx} className={`mb-1 p-1 rounded ${isDarkMode ? 'hover:bg-gray-600' : 'hover:bg-gray-200'} ${targetLanguage === 'ar' ? 'text-right' : 'text-left'}`}>
                      {example}
                    </li>
                  ))}
                </ul>
            ...
            <div className="flex justify-end mt-6">
              <button
                onClick={() => setSelectedWordForExplanation(null)}
                className="bg-indigo-500 hover:bg-indigo-600 text-white font-semibold py-2 px-4 rounded-lg shadow-md transition-colors duration-200"
              >
                إغلاق
              </button>
            ...
            {/* Resize Handle */}
            <div
              onMouseDown={startWordModalResize}
      ...
      {/* Quiz Modal */}
      {showQuizModal ? (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 overflow-y-auto py-8">
          <div
            ref={quizModalRef}
            className={`absolute p-8 rounded-xl shadow-2xl ${isDarkMode ? 'bg-gray-800 text-gray-100' : 'bg-white text-gray-900'}`}
            style={{
          ...
          >
            ...
            <h2 className="text-2xl font-bold mb-6 text-indigo-600 relative z-20" dir="rtl">اختبار القصة</h2>
            {quizQuestions.length > 0 ? (
              <div className="overflow-y-auto" style={{ maxHeight: quizModalHeight === 'auto' ? 'none' : `${quizModalHeight - 180}px` }}>
            ...
            {/* Resize Handle */}
            <div
              onMouseDown={startQuizModalResize}
              className="absolute bottom-0 right-0 w-4 h-4 bg-indigo-600 cursor-nwse-resize rounded-br-lg"
              style={{ zIndex: 10 }}
            ></div>
          </div>
      ...
      {showRephraseModal ? (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50">
          <div
            ref={rephraseModalRef}
            className={`absolute p-8 rounded-xl shadow-2xl ${isDarkMode ? 'bg-gray-800 text-gray-100' : 'bg-white text-gray-900'}`}
            style={{
              width: rephraseModalWidth,
              height: rephraseModalHeight,
              minWidth: '300px',
              minHeight: '200px',
              maxWidth: '90vw',
              maxHeight: '90vh',
          ...
          >
            {/* Drag Handle */}
            <div onMouseDown={startRephraseModalDrag} className="absolute top-0 left-0 right-0 h-10 cursor-grab rounded-t-xl z-10"></div>
            <h2 className="text-2xl font-bold mb-4 text-indigo-600 relative z-20" dir="rtl">إعادة صياغة الجملة</h2>
            {isRephrasing ? (
              <div className="flex items-center justify-center py-8" dir="rtl">
                <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-indigo-500"></div>
                <p className="ml-4">جاري إعادة الصياغة...</p>
              </div>
            ...
            {/* Resize Handle */}
            <div
              onMouseDown={startRephraseModalResize}
              className="absolute bottom-0 right-0 w-4 h-4 bg-indigo-600 cursor-nwse-resize rounded-br-lg"
              style={{ zIndex: 10 }}
            ></div>
          </div>
      ...
      {showGrammarModal ? (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 overflow-y-auto py-8">
          <div
            ...
            className={`absolute p-8 rounded-xl shadow-2xl ${isDarkMode ? 'bg-gray-800 text-gray-100' : 'bg-white text-gray-900'}`}
            style={{
              width: grammarModalWidth,
              height: grammarModalHeight,
              minWidth: '300px',
              minHeight: '200px',
              maxWidth: '90vw',
              maxHeight: '90vh',
          ...
          >
            {/* Drag Handle */}
            <div onMouseDown={startGrammarModalDrag} className="absolute top-0 left-0 right-0 h-10 cursor-grab rounded-t-xl z-10"></div>
            <h2 className="text-2xl font-bold mb-4 text-indigo-600 relative z-20" dir="rtl">فحص القواعد النحوية</h2>
            {isAnalyzingGrammar ? (
              <div className="flex items-center justify-center py-8" dir="rtl">
                <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-indigo-500"></div>
                <p className="ml-4">جاري الفحص...</p>
              </div>
            ) : grammarAnalysis ? (
            ...
            {/* Resize Handle */}
            <div
              onMouseDown={startGrammarModalResize}
              className="absolute bottom-0 right-0 w-4 h-4 bg-indigo-600 cursor-nwse-resize rounded-br-lg"
              style={{ zIndex: 10 }}
            ></div>
          </div>
      ...
      {showCompletionModal ? (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50">
          <div
            ...
            className={`absolute p-8 rounded-xl shadow-2xl ${isDarkMode ? 'bg-gray-800 text-gray-100' : 'bg-white text-gray-900'}`}
            style={{
              width: completionModalWidth,
              height: completionModalHeight,
              minWidth: '300px',
              minHeight: '200px',
              maxWidth: '90vw',
              maxHeight: '90vh',
              left: completionModalX,
              top: completionModalY,
              zIndex: 50,
            }}
          >
            {/* Drag Handle */}
            <div onMouseDown={startCompletionModalDrag} className="absolute top-0 left-0 right-0 h-10 cursor-grab rounded-t-xl z-10"></div>
            <h2 className="text-2xl font-bold mb-4 text-indigo-600 relative z-20" dir="rtl">إكمال الجملة</h2>
            {isCompletingSentence ? (
              <div className="flex items-center justify-center py-8" dir="rtl">
                <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-indigo-500"></div>
                <p className="ml-4">جاري جلب الاقتراحات...</p>
              </div>
            ...
            {/* Resize Handle */}
            <div
              onMouseDown={startCompletionModalResize}
              className="absolute bottom-0 right-0 w-4 h-4 bg-indigo-600 cursor-nwse-resize rounded-br-lg"
              style={{ zIndex: 10 }}
            ></div>
          </div>
      ...
      {showCulturalInsightModal ? (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50">
          <div
            ref={culturalModalRef}
            className={`absolute p-8 rounded-xl shadow-2xl ${isDarkMode ? 'bg-gray-800 text-gray-100' : 'bg-white text-gray-900'}`}
            style={{
              width: culturalModalWidth,
              height: culturalModalHeight,
              minWidth: '300px',
              minHeight: '200px',
              maxWidth: '90vw',
              maxHeight: '90vh',
          ...
          >
            {/* Drag Handle */}
            <div onMouseDown={startCulturalModalDrag} className="absolute top-0 left-0 right-0 h-10 cursor-grab rounded-t-xl z-10"></div>
            <h2 className="text-2xl font-bold mb-4 text-indigo-600 relative z-20" dir="rtl">{culturalInsight?.title || 'رؤى ثقافية'}</h2>
            {isGettingCulturalInsight ? (
              <div className="flex items-center justify-center py-8" dir="rtl">
                <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-indigo-500"></div>
            ...
            <div className="flex justify-end mt-6">
              <button
                onClick={() => setShowCulturalInsightModal(false)}
                className="bg-indigo-500 hover:bg-indigo-600 text-white font-semibold py-2 px-4 rounded-lg shadow-md transition-colors duration-200"
              >
                إغلاق
              </button>
            ...
            {/* Resize Handle */}
            <div
              onMouseDown={startCulturalModalResize}
              className="absolute bottom-0 right-0 w-4 h-4 bg-indigo-600 cursor-nwse-resize rounded-br-lg"
              style={{ zIndex: 10 }}
            ></div>
          </div>
      ...
      {showSummaryModal ? (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50">
          ...
          >
            {/* Drag Handle */}
            <div onMouseDown={startSummaryModalDrag} className="absolute top-0 left-0 right-0 h-10 cursor-grab rounded-t-xl z-10"></div>
            <h2 className="text-2xl font-bold mb-4 text-indigo-600 relative z-20" dir="rtl">ملخص القصة</h2>
            {isSummarizing ? (
              <div className="flex items-center justify-center py-8" dir="rtl">
                <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-indigo-500"></div>
                <p className="ml-4">جاري التلخيص...</p>
              </div>
            ) : storySummary ? (
              <p className="whitespace-pre-wrap overflow-y-auto" style={{ maxHeight: summaryModalHeight === 'auto' ? 'none' : `${summaryModalHeight - 120}px` }} dir={targetLanguage === 'ar' ? 'rtl' : 'ltr'}>{storySummary}</p>
            ) : (
            ...
            {/* Resize Handle */}
            <div
              onMouseDown={startSummaryModalResize}
              className="absolute bottom-0 right-0 w-4 h-4 bg-indigo-600 cursor-nwse-resize rounded-br-lg"
              style={{ zIndex: 10 }}
            ></div>
          </div>
      ...
      {showVocabularyModal ? (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 overflow-y-auto py-8">
          <div
            ref={vocabularyModalRef}
            className={`absolute p-8 rounded-xl shadow-2xl ${isDarkMode ? 'bg-gray-800 text-gray-100' : 'bg-white text-gray-900'}`}
            style={{
              width: vocabularyModalWidth,
              height: vocabularyModalHeight,
              minWidth: '300px',
              minHeight: '200px',
              maxWidth: '90vw',
              maxHeight: '90vh',
              left: vocabularyModalX,
              top: vocabularyModalY,
              zIndex: 50,
            }}
          >
            {/* Drag Handle */}
            <div onMouseDown={startVocabularyModalDrag} className="absolute top-0 left-0 right-0 h-10 cursor-grab rounded-t-xl z-10"></div>
            <h2 className="text-2xl font-bold mb-4 text-indigo-600 relative z-20" dir="rtl">المفردات الرئيسية</h2>
            {isExtractingVocabulary ? (
              <div className="flex items-center justify-center py-8" dir="rtl">
                <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-indigo-500"></div>
                <p className="ml-4">جاري استخراج المفردات...</p>
              </div>
            ) : keyVocabulary.length > 0 ? (
              <div className="space-y-4 overflow-y-auto" style={{ maxHeight: vocabularyModalHeight === 'auto' ? 'none' : `${vocabularyModalHeight - 150}px` }}>
            ...
            {/* Resize Handle */}
            <div
              onMouseDown={startVocabularyModalResize}
              className="absolute bottom-0 right-0 w-4 h-4 bg-indigo-600 cursor-nwse-resize rounded-br-lg"
              style={{ zIndex: 10 }}
            ></div>
          </div>
      ...
      {showDialogueModal ? (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50">
          <div
            ...
            className={`absolute p-8 rounded-xl shadow-2xl flex flex-col ${isDarkMode ? 'bg-gray-800 text-gray-100' : 'bg-white text-gray-900'}`}
            style={{
              width: dialogueModalWidth,
              height: dialogueModalHeight,
              minWidth: '300px',
              minHeight: '300px',
              maxWidth: '90vw',
              maxHeight: '90vh',
          ...
          >
            {/* Drag Handle */}
            <div onMouseDown={startDialogueModalDrag} className="absolute top-0 left-0 right-0 h-10 cursor-grab rounded-t-xl z-10"></div>
            <h2 className="text-2xl font-bold mb-4 text-indigo-600 relative z-20" dir="rtl">ممارسة المحادثة ✨</h2>
            <div ref={dialogueChatRef} className={`flex-grow overflow-y-auto p-4 rounded-lg mb-4 border ${isDarkMode ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-200'}`}>
              {isRespondingToDialogue && conversationHistory.length === 0 ? ( // Show loading for initial message
                 <div className="flex justify-start mb-3" dir={targetLanguage === 'ar' ? 'rtl' : 'ltr'}>
                    <div className={`p-3 rounded-lg bg-gray-200 text-gray-800 dark:bg-gray-600 dark:text-gray-100 animate-pulse`}>
                        ...جاري بدء المحادثة باللغة المحددة
                    </div>
                </div>
              ...
              {isRespondingToDialogue && conversationHistory.length > 0 ? ( // Show loading for subsequent messages
                <div className="flex justify-start mb-3" dir={targetLanguage === 'ar' ? 'rtl' : 'ltr'}>
                  <div className={`p-3 rounded-lg bg-gray-200 text-gray-800 dark:bg-gray-600 dark:text-gray-100 animate-pulse`}>
                    ...جاري الرد
                  </div>
            ...
            <div className="flex">
              <input
                ...
                onKeyPress={(e) => { if (e.key === 'Enter') sendDialogueMessage(); }}
              ...
              <button
                ...
                disabled={!userDialogueInput.trim() || isRespondingToDialogue}
                className="bg-indigo-500 hover:bg-indigo-600 text-white font-semibold py-3 px-6 rounded-r-lg shadow-md transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                إرسال
              </button>
            </div>
            ...
            {/* Resize Handle */}
            <div
              onMouseDown={startDialogueModalResize}
              className="absolute bottom-0 right-0 w-4 h-4 bg-indigo-600 cursor-nwse-resize rounded-br-lg"
              style={{ zIndex: 10 }}
            ></div>
          </div>
      ...
      {showPronunciationModal ? (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50">
          <div
            ref={pronunciationModalRef}
            className={`absolute p-8 rounded-xl shadow-2xl ${isDarkMode ? 'bg-gray-800 text-gray-100' : 'bg-white text-gray-900'}`}
            style={{
              width: pronunciationModalWidth,
              height: pronunciationModalHeight,
              minWidth: '300px',
              minHeight: '250px',
              maxWidth: '90vw',
              maxHeight: '90vh',
              left: pronunciationModalX,
              top: pronunciationModalY,
              zIndex: 50,
            }}
          >
            {/* Drag Handle */}
            <div onMouseDown={startPronunciationModalDrag} className="absolute top-0 left-0 right-0 h-10 cursor-grab rounded-t-xl z-10"></div>
            <h2 className="text-2xl font-bold mb-4 text-indigo-600 relative z-20" dir="rtl">دليل النطق</h2>
            <div className="mb-4">
              <label htmlFor="pronunciationWord" className="block text-sm font-medium mb-1">الكلمة/العبارة:</label>
              <input
                type="text"
                id="pronunciationWord"
                value={wordForPronunciation}
            ...
            <button
              onClick={getPronunciationGuide}
              disabled={isGettingPronunciation || !wordForPronunciation.trim()}
              className="bg-indigo-500 hover:bg-indigo-600 text-white font-semibold py-2 px-4 rounded-lg shadow-md transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed mb-4"
            >
            ...
            {isGettingPronunciation ? (
              <div className="flex items-center justify-center py-8" dir="rtl">
                <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-indigo-500"></div>
                <p className="ml-4">جاري جلب دليل النطق...</p>
              </div>
            ) : pronunciationGuide ? (
            ...
            {/* Resize Handle */}
            <div
              onMouseDown={startPronunciationModalResize}
              className="absolute bottom-0 right-0 w-4 h-4 bg-indigo-600 cursor-nwse-resize rounded-br-lg"
              style={{ zIndex: 10 }}
            ></div>
          </div>
      ...
      {showRolePlayModal ? (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50">
          <div
            ref={rolePlayModalRef}
            className={`absolute p-8 rounded-xl shadow-2xl flex flex-col ${isDarkMode ? 'bg-gray-800 text-gray-100' : 'bg-white text-gray-900'}`}
            style={{
              width: rolePlayModalWidth,
              height: rolePlayModalHeight,
              minWidth: '300px',
              minHeight: '300px',
              maxWidth: '90vw',
              maxHeight: '90vh',
          ...
          >
            {/* Drag Handle */}
            <div onMouseDown={startRolePlayModalDrag} className="absolute top-0 left-0 right-0 h-10 cursor-grab rounded-t-xl z-10"></div>
            <h2 className="text-2xl font-bold mb-4 text-indigo-600 relative z-20" dir="rtl">لعب الأدوار</h2>
            <div className="mb-4">
              ...
              >
                ...
                <option value="directions">طلب الاتجاهات</option>
                <option value="shopping">التسوق</option>
              </select>
            </div>
            <button
              onClick={startRolePlayScenario}
              disabled={!selectedScenario || isRolePlayingResponding}
              className="bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-4 rounded-lg shadow-md transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed mb-4"
            >
              {isRolePlayingResponding ? 'جاري التحميل...' : 'بدء السيناريو'}
            </button>
            ...
            <div className="flex">
              <input
                type="text"
                value={rolePlayUserInput}
                onChange={(e) => setRolePlayUserInput(e.target.value)}
                onKeyPress={(e) => { if (e.key === 'Enter') sendRolePlayMessage(); }}
              ...
              <button
                ...
                disabled={!rolePlayUserInput.trim() || isRolePlayingResponding || !selectedScenario}
                className="bg-indigo-500 hover:bg-indigo-600 text-white font-semibold py-3 px-6 rounded-r-lg shadow-md transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                إرسال
              </button>
            ...
            {/* Resize Handle */}
            <div
              onMouseDown={startRolePlayModalResize}
              className="absolute bottom-0 right-0 w-4 h-4 bg-indigo-600 cursor-nwse-resize rounded-br-lg"
              style={{ zIndex: 10 }}
            ></div>
          </div>
      ...
      {showFlashcardModal ? (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50">
          <div
            ref={flashcardModalRef}
            className={`absolute p-8 rounded-xl shadow-2xl flex flex-col ${isDarkMode ? 'bg-gray-800 text-gray-100' : 'bg-white text-gray-900'}`}
            style={{
              width: flashcardModalWidth,
              height: flashcardModalHeight,
              minWidth: '400px',
              minHeight: '300px',
              maxWidth: '90vw',
              maxHeight: '90vh',
              left: flashcardModalX,
              top: flashcardModalY,
              zIndex: 50,
            }}
          >
            {/* Drag Handle */}
            <div onMouseDown={startFlashcardModalDrag} className="absolute top-0 left-0 right-0 h-10 cursor-grab rounded-t-xl z-10"></div>
            <h2 className="text-2xl font-bold mb-4 text-indigo-600 relative z-20" dir="rtl">مولد البطاقات التعليمية</h2>
            <div className="mb-4">
            ...
            <button
              ...
              disabled={isGeneratingFlashcards || (!flashcardInput.trim() && keyVocabulary.length === 0)}
              className="bg-indigo-500 hover:bg-indigo-600 text-white font-semibold py-2 px-4 rounded-lg shadow-md transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed mb-4"
            >
            ...
            {isGeneratingFlashcards ? (
              <div className="flex items-center justify-center py-8" dir="rtl">
                <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-indigo-500"></div>
                <p className="ml-4">جاري توليد البطاقات...</p>
              </div>
            ) : flashcards.length > 0 ? (
              <div className="flex-grow flex flex-col items-center justify-center p-4 rounded-lg border bg-gray-50 dark:bg-gray-700 border-gray-200 dark:border-gray-600 transition-all duration-300 relative overflow-hidden"
                   style={{ minHeight: '200px', maxHeight: flashcardModalHeight === 'auto' ? 'none' : `${flashcardModalHeight - 250}px` }}>
                ...
                <button
                  ...
                  className="mt-6 bg-purple-500 hover:bg-purple-600 text-white font-semibold py-2 px-4 rounded-lg shadow-md transition-colors duration-200"
                >
            ...
            {/* Resize Handle */}
            <div
              onMouseDown={startFlashcardModalResize}
              className="absolute bottom-0 right-0 w-4 h-4 bg-indigo-600 cursor-nwse-resize rounded-br-lg"
              style={{ zIndex: 10 }}
            ></div>
          </div>
      ...
      {showSettings ? (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50">
          ...
          >
            {/* Drag Handle */}
            <div onMouseDown={startSettingsModalDrag} className="absolute top-0 left-0 right-0 h-10 cursor-grab rounded-t-xl z-10"></div>
            <h2 className="text-2xl font-bold mb-6 text-indigo-600 relative z-20" dir="rtl">الإعدادات</h2>
            <div className="mb-6 overflow-y-auto" style={{ maxHeight: settingsModalHeight === 'auto' ? 'none' : `${settingsModalHeight - 180}px` }}>
            ...
            {/* Resize Handle */}
            <div
              onMouseDown={startSettingsModalResize}
              className="absolute bottom-0 right-0 w-4 h-4 bg-indigo-600 cursor-nwse-resize rounded-br-lg"
              style={{ zIndex: 10 }}
            ></div>
          </div>
      ...
      {showErrorModal ? (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50">
          ...
          >
            {/* Drag Handle */}
            <div onMouseDown={startErrorModalDrag} className="absolute top-0 left-0 right-0 h-10 cursor-grab rounded-t-xl z-10"></div>
            <h2 className="text-xl font-bold mb-4 text-red-600 relative z-20" dir="rtl">خطأ / تنبيه</h2>
            <p className="mb-6 text-center overflow-y-auto" style={{ maxHeight: errorModalHeight === 'auto' ? 'none' : `${errorModalHeight - 120}px` }} dir="rtl">{errorMessage}</p>
            ...
            {/* Resize Handle */}
            <div
              onMouseDown={startErrorModalResize}
              className="absolute bottom-0 right-0 w-4 h-4 bg-indigo-600 cursor-nwse-resize rounded-br-lg"
              style={{ zIndex: 10 }}
            ></div>
          </div>
